...
def setup_wildcard_tunnel_route(env, dry_run=False, yes=False):
    ...
    r.raise_for_status()
    # Tunnel may have no config yet — default everything to empty
    result = r.json().get("result") or {}
    current_config = result.get("config") or {}
    ...
    # Check if wildcard is already there — exit early if set
    has_wildcard = any(rule.get("hostname") == WILDCARD_HOSTNAME for rule in current_ingress)
    if has_wildcard:
    ...
    per_instance = [rule for rule in current_ingress
                    if rule.get("hostname") and rule.get("hostname") != WILDCARD_HOSTNAME]
...
def main():
    ...
    setup_wildcard_tunnel_route(env, dry_run=args.dry_run, yes=args.yes)
